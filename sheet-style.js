...
function conditionalText(sheetId, col, text, bg, fg = COLORS.white) {
  return { addConditionalFormatRule: { index: 0, rule: { ranges: [{ sheetId, startRowIndex: 1, startColumnIndex: col, endColumnIndex: col + 1 }], booleanRule: { condition: { type: "TEXT_EQ", values: [{ userEnteredValue: text }] }, format: { backgroundColorStyle: rgb(bg), textFormat: { foregroundColorStyle: rgb(fg), bold: true } } } } } };
}
...
async function formatSpreadsheet(sheets, spreadsheetId, TABS) {
  ...
  for (const [title,cfg] of Object.entries(schemas)) {
    ...
    requests.push({ updateSheetProperties: { properties: { sheetId:p.sheetId, gridProperties:{ frozenRowCount:1, hideGridlines:true }, tabColorStyle:rgb(title===TABS.settings?COLORS.gray:title===TABS.members?COLORS.purple:COLORS.red) }, fields:"gridProperties.frozenRowCount,gridProperties.hideGridlines,tabColorStyle" } }, header(p.sheetId,cfg.cols), body(p.sheetId,cfg.cols), rowHeight(p.sheetId,0,38));
    cfg.widths.forEach((w,i)=>requests.push(width(p.sheetId,i,w)));
  }
  const payments=byTitle.get(TABS.payments); if(payments) requests.push(currency(payments.sheetId,4,5),datetime(payments.sheetId,6,8),conditionalText(payments.sheetId,5,"APPROVED",COLORS.greenDark,COLORS.green),conditionalText(payments.sheetId,5,"REVERSED",COLORS.redDark,{red:1,green:.35,blue:.35}));
  const expenses=byTitle.get(TABS.expenses); if(expenses) requests.push(currency(expenses.sheetId,2,3),currency(expenses.sheetId,7,8),datetime(expenses.sheetId,1,2),conditionalText(expenses.sheetId,8,"ACTIVE",COLORS.greenDark,COLORS.green),conditionalText(expenses.sheetId,8,"REVERSED",COLORS.redDark,{red:1,green:.35,blue:.35}));
  const transactions=byTitle.get(TABS.transactions); if(transactions) requests.push(currency(transactions.sheetId,3,4),currency(transactions.sheetId,8,9),datetime(transactions.sheetId,1,2));
  const logs=byTitle.get(TABS.logs); if(logs) requests.push(datetime(logs.sheetId,0,1));
  const members=byTitle.get(TABS.members); if(members) requests.push(conditionalText(members.sheetId,4,"BETAALD",COLORS.greenDark,COLORS.green),conditionalText(members.sheetId,4,"OPENSTAAND",COLORS.amberDark,COLORS.amber));

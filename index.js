...
const TABS = { dashboard: "Dashboard", payments: "Weekbetalingen", expenses: "Uitgaven", transactions: "Transacties", logs: "Logs", settings: "Settings" };
...
async function ensureSheets() {
  ...
  const headers = [
    [TABS.dashboard, "A1:B6", [["Sondravo Family Dashboard", ""],["Laatste update", ""],["Huidig saldo", 0],["Totaal inkomsten", 0],["Totaal uitgaven", 0],["Weekpot tarief", CFG.weeklyAmount]]],
...
function familyMembers(guild) { return guild.members.cache.filter(m => !m.user.bot && (m.roles.cache.has(CFG.familyRoleId) || m.roles.cache.has(CFG.mpikambanaRoleId) || m.roles.cache.has(CFG.founderRoleId))); }
...
async function registerWeekPayment(member, week, actor) {
  ...
  await addLog("WEEKBETALING_AANGEMAAKT",actor,`${member.user.username} • ${week} • ${money(CFG.weeklyAmount)}`,paymentId); return {paymentId,balance};
}
...
async function fullReset(actor) {
  ...
  await clearTabData(TABS.payments); await clearTabData(TABS.expenses); await clearTabData(TABS.transactions); await clearTabData(TABS.logs);
  ...
  await sheets.spreadsheets.values.update({spreadsheetId:CFG.spreadsheetId,range:`${TABS.dashboard}!A2:B6`,valueInputOption:"USER_ENTERED",requestBody:{values:[["Laatste update",new Date().toISOString()],["Huidig saldo",0],["Totaal inkomsten",0],["Totaal uitgaven",0],["Weekpot tarief",CFG.weeklyAmount]]}});
  await addLog("VOLLEDIGE_RESET",actor,"Alle testdata gewist; administratie opnieuw gestart vanaf 0.","reset");
}

function dashboardButtons(){return[
  new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sf:pot").setLabel("Gangpot").setEmoji("💰").setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId("sf:week").setLabel("Weekpot beheren").setEmoji("💵").setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId("sf:expense").setLabel("Uitgave").setEmoji("💸").setStyle(ButtonStyle.Danger)),
  new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sf:members").setLabel("Leden").setEmoji("👥").setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId("sf:overview").setLabel("Overzicht").setEmoji("📊").setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId("sf:logs").setLabel("Logs").setEmoji("📜").setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId("sf:admin").setLabel("Founder beheer").setEmoji("⚙️").setStyle(ButtonStyle.Secondary))
];}
async function dashboardData(guild){const t=await totals(),members=familyMembers(guild),week=await activeWeek(),rows=await getRows(TABS.payments,"A2:J"),paidIds=new Set(rows.filter(r=>r[1]===week&&String(r[5]||"").toUpperCase()==="APPROVED").map(r=>r[2])),paid=[...members.values()].filter(m=>paidIds.has(m.id)).length; await sheets.spreadsheets.values.update({spreadsheetId:CFG.spreadsheetId,range:`${TABS.dashboard}!A2:B6`,valueInputOption:"USER_ENTERED",requestBody:{values:[["Laatste update",new Date().toISOString()],["Huidig saldo",t.balance],["Totaal inkomsten",t.income],["Totaal uitgaven",t.expense],["Weekpot tarief",CFG.weeklyAmount]]}}); return {...t,totalMembers:members.size,paid,week};}
function dashboardEmbed(data){const open=Math.max(0,data.totalMembers-data.paid);return baseEmbed().setTitle(`🩸 ${CFG.gangName.toUpperCase()}`).setDescription("**Management Panel**\nAlleen de leiding registreert weekbetalingen en uitgaven.").addFields({name:"💰 Gangpot",value:`**${money(data.balance)}**`,inline:true},{name:"💵 Weekpot",value:`${money(CFG.weeklyAmount)} p.p.`,inline:true},{name:`📅 ${data.week}`,value:`✅ ${data.paid}/${data.totalMembers} betaald\n⏳ ${open} openstaand`,inline:true},{name:"📈 Inkomsten",value:money(data.income),inline:true},{name:"📉 Uitgaven",value:money(data.expense),inline:true},{name:"🔒 Beheer",value:"Founder kan weken en transacties beheren.",inline:true});}
async function refreshDashboard(guild){const channelId=await getSetting("dashboard_channel_id"),messageId=await getSetting("dashboard_message_id");if(!channelId||!messageId)return;try{const ch=await guild.channels.fetch(channelId),msg=await ch.messages.fetch(messageId);await msg.edit({embeds:[dashboardEmbed(await dashboardData(guild))],components:dashboardButtons()});}catch{}}

const commands=[new SlashCommandBuilder().setName("setup").setDescription("Plaats het Sondravo management dashboard."),new SlashCommandBuilder().setName("saldo").setDescription("Bekijk de huidige gangpot."),new SlashCommandBuilder().setName("correctie").setDescription("Founder: corrigeer de gangpot.").addStringOption(o=>o.setName("richting").setDescription("Plus of min").setRequired(true).addChoices({name:"Plus",value:"plus"},{name:"Min",value:"min"})).addNumberOption(o=>o.setName("bedrag").setDescription("Bedrag").setRequired(true).setMinValue(1)).addStringOption(o=>o.setName("reden").setDescription("Reden").setRequired(true))].map(c=>c.toJSON());

const client=new Client({intents:[GatewayIntentBits.Guilds,GatewayIntentBits.GuildMembers]});
client.once(Events.ClientReady,async c=>{console.log(`✅ Online als ${c.user.tag}`);await ensureSheets();const rest=new REST({version:"10"}).setToken(CFG.token);await rest.put(Routes.applicationGuildCommands(CFG.clientId,CFG.guildId),{body:commands});console.log("✅ Slash commands geregistreerd.");const guild=c.guilds.cache.get(CFG.guildId);if(guild)await refreshDashboard(guild);});

client.on(Events.InteractionCreate,async i=>{try{
  if(i.isChatInputCommand()){requireFamily(i);if(i.commandName==="setup"){requireFounder(i);await i.deferReply({ephemeral:true});const msg=await i.channel.send({embeds:[dashboardEmbed(await dashboardData(i.guild))],components:dashboardButtons()});await setSetting("dashboard_channel_id",i.channel.id);await setSetting("dashboard_message_id",msg.id);await addLog("SETUP",i.user,`Dashboard geplaatst in #${i.channel.name}`,msg.id);await i.editReply("✅ Dashboard geplaatst.");return;}if(i.commandName==="saldo"){const t=await totals();await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle("💰 Sondravo Gangpot").setDescription(`Huidig saldo: **${money(t.balance)}**`)]});return;}if(i.commandName==="correctie"){requireFounder(i);await i.deferReply({ephemeral:true});const richting=i.options.getString("richting"),bedrag=i.options.getNumber("bedrag"),reden=i.options.getString("reden"),txId=id("corr"),balance=await addTransaction({id:txId,type:richting==="plus"?"CORRECTIE_PLUS":"CORRECTIE_MIN",amount:bedrag,userId:i.user.id,userName:i.user.username,description:reden,reference:"manual"});await addLog("CORRECTIE",i.user,`${richting} ${money(bedrag)} • ${reden}`,txId);await refreshDashboard(i.guild);await i.editReply(`✅ Correctie verwerkt. Nieuw saldo: **${money(balance)}**`);return;}}
  if(i.isButton()){requireFamily(i);
    if(i.customId==="sf:pot"){const t=await totals();await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle("💰 Gangpot").setDescription(`Beschikbaar: **${money(t.balance)}**`).addFields({name:"📈 Totaal binnen",value:money(t.income),inline:true},{name:"📉 Totaal eruit",value:money(t.expense),inline:true})]});return;}
    if(i.customId==="sf:week"){requireFounder(i);const week=await activeWeek(),members=familyMembers(i.guild),rows=await getRows(TABS.payments,"A2:J"),paidIds=new Set(rows.filter(r=>r[1]===week&&String(r[5]||"").toUpperCase()==="APPROVED").map(r=>r[2])),paid=[...members.values()].filter(m=>paidIds.has(m.id)).length,open=Math.max(0,members.size-paid),button=new ButtonBuilder().setCustomId(`sf:week:new:${week}`).setLabel("Nieuwe betaling aanmaken").setEmoji("➕").setStyle(ButtonStyle.Success).setDisabled(open===0);await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle(`💵 Weekpot beheren • ${week}`).setDescription(`Bedrag per persoon: **${money(CFG.weeklyAmount)}**\n✅ Betaald: **${paid}/${members.size}**\n⏳ Openstaand: **${open}**\n\nKlik op **Nieuwe betaling aanmaken** en kies daarna de persoon die betaald heeft.`)],components:[new ActionRowBuilder().addComponents(button)]});return;}
    if(i.customId.startsWith("sf:week:new:")){requireFounder(i);const week=i.customId.split(":")[3],members=familyMembers(i.guild),rows=await getRows(TABS.payments,"A2:J"),paidIds=new Set(rows.filter(r=>r[1]===week&&String(r[5]||"").toUpperCase()==="APPROVED").map(r=>r[2])),unpaid=[...members.values()].filter(m=>!paidIds.has(m.id)).sort((a,b)=>a.displayName.localeCompare(b.displayName));if(!unpaid.length){await i.update({content:`✅ Iedereen staat voor **${week}** op betaald.`,embeds:[],components:[]});return;}const options=unpaid.slice(0,25).map(m=>({label:m.displayName.slice(0,100),description:`${money(CFG.weeklyAmount)} weekbetaling`,value:m.id})),select=new StringSelectMenuBuilder().setCustomId(`sf:week:pick:${week}`).setPlaceholder("Kies één persoon die heeft betaald...").addOptions(options);await i.update({embeds:[baseEmbed().setTitle("➕ Nieuwe weekbetaling").setDescription(`Week: **${week}**\nBedrag: **${money(CFG.weeklyAmount)}**\n\nKies nu de persoon die betaald heeft.`)],components:[new ActionRowBuilder().addComponents(select)]});return;}
    if(i.customId==="sf:expense"){requireFounder(i);const modal=new ModalBuilder().setCustomId("sf:modal:expense").setTitle("Nieuwe Sondravo uitgave"),amount=new TextInputBuilder().setCustomId("amount").setLabel("Bedrag").setPlaceholder("Bijv. 450000").setStyle(TextInputStyle.Short).setRequired(true),category=new TextInputBuilder().setCustomId("category").setLabel("Categorie").setPlaceholder("Auto / Wapen / Event / Overig").setStyle(TextInputStyle.Short).setRequired(true),desc=new TextInputBuilder().setCustomId("description").setLabel("Omschrijving").setPlaceholder("Bijv. Sultan RS voor de family").setStyle(TextInputStyle.Paragraph).setRequired(true);modal.addComponents(new ActionRowBuilder().addComponents(amount),new ActionRowBuilder().addComponents(category),new ActionRowBuilder().addComponents(desc));await i.showModal(modal);return;}
    if(i.customId==="sf:members"){requireFounder(i);const members=familyMembers(i.guild),week=await activeWeek(),rows=await getRows(TABS.payments,"A2:J"),paid=new Set(rows.filter(r=>r[1]===week&&String(r[5]||"").toUpperCase()==="APPROVED").map(r=>r[2])),lines=[...members.values()].sort((a,b)=>a.displayName.localeCompare(b.displayName)).slice(0,40).map(m=>`${paid.has(m.id)?"✅":"⏳"} <@${m.id}>`);await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle(`👥 Leden • ${week}`).setDescription(lines.join("\n")||"Geen family-leden gevonden.")]});return;}
    ...
    if(i.customId==="sf:logs"){requireFounder(i);const rows=(await getRows(TABS.logs,"A2:F")).slice(-15).reverse(),lines=rows.map(r=>`**${r[1]}** • ${r[4]||"-"}\n> door ${r[3]||r[2]||"onbekend"}`);await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle("📜 Beheerlogs").setDescription(lines.join("\n\n")||"Nog geen logs.")]});return;}
    if(i.customId==="sf:admin"){requireFounder(i);const week=await activeWeek(),row1=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sf:admin:newweek").setLabel("Nieuwe week starten").setEmoji("📅").setStyle(ButtonStyle.Primary),new ButtonBuilder().setCustomId("sf:admin:revertpay").setLabel("Betaling terugdraaien").setEmoji("↩️").setStyle(ButtonStyle.Secondary),new ButtonBuilder().setCustomId("sf:admin:revertexp").setLabel("Uitgave terugdraaien").setEmoji("🗑️").setStyle(ButtonStyle.Danger)),row2=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sf:admin:reset").setLabel("Volledige reset").setEmoji("🧨").setStyle(ButtonStyle.Danger));await i.reply({ephemeral:true,embeds:[baseEmbed().setTitle("⚙️ Founder beheer").setDescription(`Actieve week: **${week}**\n\nHier beheer je weken, transacties en — indien nodig — een volledige herstart vanaf 0.`)],components:[row1,row2]});return;}
    if(i.customId==="sf:admin:newweek"){requireFounder(i);const current=await activeWeek(),next=nextWeekKey(current);await setSetting("active_week",next);await addLog("NIEUWE_WEEK",i.user,`${current} → ${next}`,next);await refreshDashboard(i.guild);await i.update({embeds:[baseEmbed().setTitle("📅 Nieuwe week gestart").setDescription(`De actieve week is nu **${next}**.\nIedereen staat weer openstaand.`)],components:[]});return;}

...
async function ensureSheets() {
  ...
  const headers = [
    ...
    [TABS.settings, "A1:B4", [["Key","Value"],["dashboard_channel_id",""],["dashboard_message_id",""],["approval_channel_id",""]]]
  ];
...
async function setSetting(key, value) {
  ...
  if (idx >= 0) {
    await sheets.spreadsheets.values.update({ spreadsheetId: CFG.spreadsheetId, range: `${TABS.settings}!B${idx+2}`, valueInputOption: "USER_ENTERED", requestBody: { values: [[String(value || "")]] } });
...
async function createPaymentRequest({ id, week, user, note }) {
  ...
  const duplicate = rows.find(r => r[1] === week && r[2] === user.id && ["PENDING","APPROVED"].includes(String(r[5] || "").toUpperCase()));
  if (duplicate) throw new Error(String(duplicate[5]).toUpperCase() === "APPROVED" ? "Je betaling voor deze week is al goedgekeurd." : "Je hebt voor deze week al een betaling in afwachting.");
  await append(TABS.payments, [id, week, user.id, user.username, CFG.weeklyAmount, "PENDING", new Date().toISOString(), "", "", note || ""]);
}
...
async function updatePayment(id, status, founder) {
  ...
  if (String(found.row[5] || "").toUpperCase() !== "PENDING") throw new Error("Deze aanvraag is al behandeld.");
  await sheets.spreadsheets.values.update({ spreadsheetId: CFG.spreadsheetId, range: `${TABS.payments}!F${found.rowNumber}:I${found.rowNumber}`, valueInputOption: "USER_ENTERED", requestBody: { values: [[status, new Date().toISOString(), founder.username, founder.id]] } });
  return found.row;
}
...
function isMpikambana(i) { return isFounder(i) || hasRole(i, CFG.mpikambanaRoleId); }
function isFamily(i) { return isFounder(i) || isMpikambana(i) || hasRole(i, CFG.familyRoleId); }
...
function requireMpikambana(i) { if (!isMpikambana(i)) throw new Error("Alleen Mpikambana of Founder kan dit gebruiken."); }
  ...
  try {
    ...
    if (i.isButton()) {
      ...
      if (i.customId === "sf:week") {
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sf:week:submit").setLabel(approved ? "Deze week al betaald" : `Betaling ${money(CFG.weeklyAmount)} aanvragen`).setEmoji(approved ? "✅" : "💵").setDisabled(approved).setStyle(approved ? ButtonStyle.Success : ButtonStyle.Primary));
        await i.reply({ ephemeral: true, embeds: [baseEmbed().setTitle("💵 Weekpot").setDescription(`Week: **${week}**\nBedrag: **${money(CFG.weeklyAmount)}**\n\n${approved ? "✅ Jouw betaling voor deze week is goedgekeurd." : "Druk hieronder nadat je hebt betaald. Founder moet hem daarna goedkeuren."}`)], components: [row] });
        return;
      }
      if (i.customId === "sf:week:submit") {
        requireMpikambana(i);
        const modal = new ModalBuilder().setCustomId("sf:modal:week").setTitle("Sondravo Weekbetaling");
        const note = new TextInputBuilder().setCustomId("note").setLabel("Opmerking (optioneel)").setPlaceholder("Bijv. cash / bank / betaald aan Founder").setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(300);
      ...
      if (i.customId === "sf:overview") {
        ...
        const lines = rows.map(r => { const minus = ["UITGAVE","CORRECTIE_MIN"].includes(String(r[2] || "").toUpperCase()); return `${minus ? "🔴" : "🟢"} **${r[2]}** ${minus ? "-" : "+"}${money(r[3])}\n> ${r[6] || "-"} • saldo ${money(r[8] || 0)}`; });
      ...
      if (i.customId.startsWith("sf:approve:")) {
        ...
        const txId = rid("tx");
        const balance = await addTransaction({ id: txId, type: "WEEKBETALING", amount: Number(amount), userId, userName, description: `Weekpot ${week}`, reference: requestId });
        await addLog("BETALING_GOEDGEKEURD", i.user, `${userName} • ${week} • ${money(amount)}`, requestId);
        await i.message.edit({ embeds: [EmbedBuilder.from(i.message.embeds[0]).setColor(0x2E8B57).setTitle("✅ Weekbetaling • Goedgekeurd")], components: [] });
        await refreshDashboard(i.guild);
      ...
      if (i.customId.startsWith("sf:reject:")) {
        ...
        await i.message.edit({ embeds: [EmbedBuilder.from(i.message.embeds[0]).setColor(0x555555).setTitle("❌ Weekbetaling • Afgewezen")], components: [] });
    ...
    if (i.isModalSubmit()) {
      ...
      if (i.customId === "sf:modal:week") {
        requireMpikambana(i);
        await i.deferReply({ ephemeral: true });
        const requestId = rid("pay");
        const week = weekKey();
        const note = i.fields.getTextInputValue("note") || "";
        await createPaymentRequest({ id: requestId, week, user: i.user, note });
        const channelId = await getSetting("approval_channel_id");
        const ch = channelId ? await i.guild.channels.fetch(channelId).catch(() => null) : i.channel;
        const embed = baseEmbed().setTitle("💵 Weekbetaling • Goedkeuring").setDescription("Een weekbetaling wacht op goedkeuring.").addFields(
        ...
        const buttons = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`sf:approve:${requestId}`).setLabel("Goedkeuren").setEmoji("✅").setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`sf:reject:${requestId}`).setLabel("Afwijzen").setEmoji("❌").setStyle(ButtonStyle.Danger)
        );
        await ch.send({ content: `<@&${CFG.founderRoleId}> nieuwe weekbetaling ter controle.`, embeds: [embed], components: [buttons], allowedMentions: { roles: [CFG.founderRoleId] } });
      ...
      if (i.customId === "sf:modal:expense") {
        ...
        await append(TABS.expenses, [txId, new Date().toISOString(), amount, category, description, i.user.id, i.user.username, balance]);
